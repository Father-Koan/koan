...
- **Tests MUST NOT post to chat (Telegram/outbox).** This is a must-follow rule. A test that exercises code which can notify the user must mock the outbound sink so nothing is delivered: patch `app.notify.send_telegram` (and/or `append_to_outbox` / `format_and_send`), or pass an explicit `notify_fn` capture. Because notifications are gated by `messaging.level`, a test that only sets the level is **not** sufficient — `messaging_level.notify_outcome()` always sends and `progress_notify()` sends in `debug`, so the sink itself must be stubbed regardless of level. Assert against the captured messages, never against a live send.
